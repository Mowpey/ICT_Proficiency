    ...
    if request.method == "POST":
        ...
        if search_entries:
            ...
            for column in UserAssessment.__table__.columns:
                ...
                elif column.name in ['exam_venue', 'venue_address', 'proctor','remarks']:
                    conditions.append(column.ilike(f"%{search_entries}%"))
            ...
            if conditions:
                stmt = (select(UserAssessment, *DiagnosticResults.__table__.columns)
                ...
                total_count = db.session.execute(select(func.count(UserAssessment.applicant_id))
                                                 .join(DiagnosticResults, UserAssessment.applicant_id == DiagnosticResults.applicant_id)
                                                 .where(or_(*conditions))).scalar() or 0
                total_pages = ceil(total_count / per_page)
                offset = (page - 1) * per_page
