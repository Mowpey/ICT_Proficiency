...
def insertDiagnosticData():
    if request.method == 'POST':
        ...
        diagnostic_form_data = insert(DiagnosticResults).values(
            ...
            date_of_examination=datetime.strptime(request.form['date_exam'], '%Y-%m-%d').date(),
            date_of_notification=datetime.strptime(request.form['date_notified'], '%Y-%m-%d').date(),
...
def insertHandsonData():
    if request.method == 'POST':
        ...
        handson_form_data = insert(HandsonResults).values(
            ...
            date_of_examination=datetime.strptime(request.form.get('date_exam', ''), '%Y-%m-%d').date(),
            date_of_notification=datetime.strptime(request.form.get('date_notified', ''), '%Y-%m-%d').date(),
