...
@filter_bp.route('/apply_filters_assessment', methods=['POST'])
def applyFilterAssessment():
    page = request.args.get('page', 1, type=int)
    per_page = 25
    ...
    if sort_date_exam:
        ...
        start_date = datetime.strptime(start_date, '%B %d, %Y').date()
        end_date = datetime.strptime(end_date, '%B %d, %Y').date()
        filtering_conditions.append(UserAssessment.date_of_examination.between(start_date, end_date))
    if sort_date_notified:
        start_date, end_date = sort_date_notified.split(' - ')
        start_date = datetime.strptime(start_date, '%B %d, %Y').date()
        end_date = datetime.strptime(end_date, '%B %d, %Y').date()
        filtering_conditions.append(UserAssessment.date_of_notification.between(start_date, end_date))
    if sort_venue:
        filtering_conditions.append(or_(
            UserAssessment.venue_address.ilike(f"%{sort_venue}%"),
            UserAssessment.exam_venue.ilike(f"%{sort_venue}%")
        ))
    if sort_proctor:
        filtering_conditions.append(UserAssessment.proctor.ilike(f"%{sort_proctor}%"))
    if sort_status:
        filtering_conditions.append(UserAssessment.status == sort_status)

    if filtering_conditions:
        filter_stmt = filter_stmt.where(and_(*filtering_conditions))
    else:
        filter_stmt = filter_stmt.order_by(desc(UserAssessment.applicant_id))
